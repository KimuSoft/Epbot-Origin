...
async def ox(bot, message, ctx, auto_delete=True):
    """
    🅾️❎ 이모지 선택지를 만들어 줘요!
    🅾️를 누르면 0를 반환해요(False)
    ❎를 누르면 1를 반환하고(True)
    만약 시간이 초과되면 2를 반환해요(True)
    ※ 헷갈림 주의
    """
...
async def wait_for_reaction(
    bot, window, canpress, timeout, ctx, event="reaction_add", add_react=True
):
    ...
    def check(reaction, user):
        if (
            user == ctx.author
            and str(reaction.emoji) in canpress
            and reaction.message.id == window.id
        ):
            return True
        else:
            return False
