...
def on_working(fishing=False, landwork=False, prohibition=False, owner_only=False, twoball=True):
    '''
        fishing : 낚시와 동시에 불가능
    ...
    async def predicate(ctx):
        ...
        if 'DM' in str(type(channel)):
            await ctx.respond(content = f'으에, 이프는 DM은 안 받고 이써!\n`❗ 이프와는 개인 메시지로 놀 수 없습니다.`')
            return False
        ...
        if landwork:  # 땅 작업 중에는 금지
            if working_now(ctx.channel.id):
                try:
                    await ctx.respond(content = '흐음... 여기 뭔가 하고 있는 거 같은데 조금 이따가 와 보자!\n`❗ 누군가 이미 땅에서 매입/매각/건설/철거 등의 작업을 하는 중이다.`')
                except Exception:
                    pass
                return False
        ...
        if prohibition:  # 낚시금지를 했다면 금지
            if channel.topic is not None and '#낚시금지' in channel.topic:
                try:
                    await ctx.respond(content = "여긴 낚시터가 아니야...\n`❗ 낚시 금지 태그가 설정된 채널입니다.`")
                except Exception:
                    pass
                return False
            ...
            if channel.topic is not None and '#no_fishing' in channel.topic:
                try:
                    await ctx.respond(content = "You can't fish here! >ㅅ<\n`❗ This channel is tagged with no fishing.`")
                except Exception:
                    pass
                return False
        ...
        if owner_only:  # 낚시터 주인만 가능
            ...
            if room.owner_id != ctx.author.id:
                try:
                    await ctx.respond(content = '다른 사람 땅은 건들 수 없어...!\n`❗ 자신의 땅에서만 할 수 있는 작업이다.`')
                except Exception:
                    pass
                return False
