...
def getCard(fish=None, room=None, user=None):
    img = Image.open(f'utils/fish_card/card_type_{fish.rarity}.png')  # 일단 기본배경폼 이미지를 open 합니다.
    ...
    text_up = {0: '{}을(를) 낚아 버렸다;;;', 1: '{}이(가) 낚였다!', 2: '와! {}을(를) 낚았다!!!', 3: '월척이야!!! {}을 낚다니!', 4: '이건... {}?!?!', 5: '{}(이)라고?!?! 가능한 거야???'}
    ...
    if fish.rarity != 0:
        if room.owner_id == user.id:
            draw.text((72, 85), f"{fee:+,}$ ({-1 * room.maintenance:+}%)", font=font16, fill=(50, 0, 0))
            draw.text((72, 38), "낚시터 주인", font=font8)
        else:
            draw.text((72, 85), f"{fee:+,}$ ({-1 * (room.fee + room.maintenance):+}%)", font=font16, fill=(50, 0, 0))
        draw.text((72, 105), f"{bonus:+,}$ ({room.bonus:+}%)", font=font16, fill=(0, 50, 0))
    ...
    draw.text((280, 170), f"{time.strftime('%Y-%m-%d %H')}시에 '{deEmojify(room.name)}'에서", font=font8, fill=(0, 0, 0))
    # img.show()
...
def getCard_eng(fish=None, room=None, user=None):
    img = Image.open(f'utils/fish_card/en_card_{fish.rarity}.png')  # 일단 기본배경폼 이미지를 open 합니다.
    draw = ImageDraw.Draw(img)
    ...
    text_up = {0: 'I caught the {} ...', 1: 'I caught a {}!', 2: 'WOW! I caught the a {}!!!', 3: "I caught a {}! I can't belive it!!!", 4: 'This is... {}?!?!', 5: '{}?! Is it possible?!?!'}
    ...
    if fish.rarity != 0:
        if room.owner_id == user.id:
            draw.text((72, 85), f"{fee:,}$ (OWNER {room.maintenance}%)", font=font16, fill=(50, 0, 0))
        else:
            draw.text((72, 85), f"{fee:,}$ ({room.fee + room.maintenance}%)", font=font16, fill=(50, 0, 0))
        draw.text((72, 105), f"+{bonus:,}$ ({room.bonus}%)", font=font16, fill=(0, 50, 0))
        draw.text((85, 145), f"{(cost + fee + bonus):,}$", font=font24, fill=(0, 0, 0))
    ...
    draw.text((350, 90), f"{fish.length}cm", font=font20, fill=(0, 0, 0))
    draw.text((350, 115), f"(Average {fish.average_length}cm)", font=font12, fill=(0, 0, 0))
    draw.text((350, 130), f"{fish.cost():,}$", font=font20, fill=(0, 0, 0))
    draw.text((350, 155), f"(Average {fish.average_cost:,}$)", font=font12, fill=(0, 0, 0))

    time = datetime.today()
    draw.text((290, 160), f"『{deEmojify(user.name)}』", font=font8, fill=(0, 0, 0))
    draw.text((280, 170), f"In the '{deEmojify(room.name)}' at {time.strftime('%Y-%m-%d %H')}", font=font8, fill=(0, 0, 0))
    # img.show()
