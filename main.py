...
class AdminCog(commands.Cog):
    ...
    async def 다시시작(self, ctx):
        if ctx.author.id not in config.ADMINS:
            return await ctx.send("흐음... 권한이 부족한 것 같은데?" "\n`❗ 권한이 부족합니다.`")

        w = await ctx.send("`❗ Cogs를 다시 불러오고 이써...`")
        ...
        logger.info("다시시작 완료!")
        await w.edit(content="`✔️ 전부 다시 불러와써!`")
    ...
    async def on_command_error(self, ctx, error):
        ...
        if not per.send_messages:  # 일단 쓸 수 없으면 아무 의미 없잖아
            logger.warn(f"{channel.name}({channel.id})에서 메시지 쓰기 권한이 없음")
        ...
        # 권한 부족 오류
            ...
            or "permissions" in str(error).replace(" ", "").lower()
        ):
            perdict = {
